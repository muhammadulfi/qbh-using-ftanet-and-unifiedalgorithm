...
def create_model(input_shape=(320, 430, 3)):
    ...
    # 保持高分辨率，关注细节
    shape=input_shape
    x_r, x_t = FTA_Module(x, (shape[0], shape[1], 32), 3, 3)
    x = SF_Module([x_r, x_t], 32, 4, 4)
    x = MaxPooling2D((2, 2))(x)

    x_r, x_t = FTA_Module(x, (shape[0]//2, shape[1]//2, 64), 3, 3)
    x = SF_Module([x_r, x_t], 64, 4, 4)
    x = MaxPooling2D((2, 2))(x)

    x_r, x_t = FTA_Module(x, (shape[0]//4, shape[1]//4, 128), 3, 3)
    x = SF_Module([x_r, x_t], 128, 4, 4)
    ...
    x_r, x_t = FTA_Module(x, (shape[0], shape[1], 1), 3, 3)
    x = SF_Module([x_r, x_t], 1, 4, 4)
