...
#olny get time for training
def getTimeArr(fpath, sr=None, hop=256, model_type='vocal', window=2049):
    y, sr = load_audio(fpath, sr=sr)
    if 'vocal' in model_type:
        Z, time, CenFreq, tfrL0, tfrLF, tfrLQ = feature_extraction(y, sr, Hop=hop, Window=window, StartFreq=31.0, StopFreq=1250.0, NumPerOct=60)
    if 'melody' in model_type:
        Z, time, CenFreq, tfrL0, tfrLF, tfrLQ = feature_extraction(y, sr, Hop=hop, Window=window, StartFreq=20.0, StopFreq=2048.0, NumPerOct=60)
...
def cfp_process(fpath, ypath=None, csv=False,sr=None, hop=256, model_type='vocal', window=2049):
